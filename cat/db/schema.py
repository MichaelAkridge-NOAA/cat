...
DDL_BLOCKS: List[str] = [
    ...
    """,
    # -----------------------------------------------------------------
    # Retry-safe annotation creates. The client stamps every new annotation
    # with a UUID before its first POST and re-sends the same UUID on every
    # retry (timeout, tab-close keepalive, journal replay), so a create that
    # already committed is returned instead of inserted again. Unique on the
    # column alone: Oracle doesn't index all-NULL keys, so the existing rows
    # (client_uuid NULL) don't collide, whereas (project_id, client_uuid)
    # would treat them as duplicate (project, NULL) pairs.
    # -----------------------------------------------------------------
    """
    BEGIN
    ...
    """,
    # -----------------------------------------------------------------
    # Annotation history: every UPDATE (edits, soft delete, restore) and
    # DELETE of a cat_annotations row first copies the OLD row here, via a
    # trigger — so it is atomic with the change and covers every write path,
    # including ones added later. Deliberately NO foreign keys: history must
    # outlive the annotation and even the project (an FK to cat_projects
    # would also make the project-delete cascade fail inside the trigger).
    # -----------------------------------------------------------------
    """
    BEGIN
        EXECUTE IMMEDIATE q'[
    ...
    END;
    """,
    # The trigger body needs :OLD/:NEW. They are written as ~OLD/~NEW and
    # turned into colons with CHR(58) at run time, so this statement contains
    # no colon for the database driver to mistake for a bind variable.
    """
    BEGIN
    ...
    """,
    # -----------------------------------------------------------------
    # Team-editable lists for the numbered condition fields: one list per
    # group ('rdcause' -> rdcause1-3, 'con' -> con_1-3, 'sev' -> sev_1-3).
    # Only severity is seeded (1-5, the range the form already enforced);
    # cause/condition codes are left empty on purpose — a team lead adds
    # them (or imports the values already in use), and until a list has
    # options those fields stay free text.
    # -----------------------------------------------------------------
    """
    MERGE INTO cat_annotation_field_options dst
    USING (
